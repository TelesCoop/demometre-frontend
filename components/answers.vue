...
const props = defineProps({
  answers: { type: Array as PropType<Array<AnswerType>>, required: true },
  modelValue: { type: Number, required: false },
})
const isAnswerSelected = computed(
  () => (answerId) => answerId === props.modelValue
)
...
// handle keyboard
const nextAnswer = (ev) => {
  const currentAnswerIndex = props.answers.findIndex(
    (answer) => answer.id === props.modelValue
  )
